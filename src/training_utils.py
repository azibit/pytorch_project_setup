...
def train_model(model, dataloader, epoch, training_function, criterion_to_use, optimizer_to_use):
    ...
    with tqdm(dataloader, unit = 'batch') as tepoch:
        ...
        # for batch_idx, (inputs, targets) in enumerate(dataloader):
            ...
            tepoch.set_postfix(loss=loss, accuracy=(100. * correct/total).item())
...
def test_model(model, dataloader, criterion_to_use, print_classification_report = False):
    ...
    for batch_idx, (inputs, targets) in enumerate(dataloader):
        ...
        # progress_bar(batch_idx, len(dataloader), 'Loss: %.3f | Acc: %.3f%% (%d/%d)' % (training_loss/(batch_idx+1), 100.*correct/total, correct, total))
